...
function calculateSectionEmissions(sectionName) {
   ...
   let selectedKeys = 0; // Array to store selected keys

   for (var key in sectionArray) {

    if(key == question1.value){
      selectedKeys += sectionArray[key];
    }

    if(key == question3.value || key == question4.value || key == question5.value || (key == question6DryerBox.value && question6DryerBox.checked) || 
   ...
   if(sectionName == "transportation"){
    question2Val = question2.value * selectedKeys;
